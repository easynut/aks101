...
if [ $? -ne 0 ]; then
  echo "Failed to install IBM Kubernetes Service CLI prerequisites"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "Failed to authenticate to IBM Kubernetes Service"
  exit 1
fi
...
. workshop/deploy.sh
if [ $? -ne 0 ]; then
  echo "Failed to Deploy pods for stage 3 to IBM Kubernetes Service"
  exit 1
fi
